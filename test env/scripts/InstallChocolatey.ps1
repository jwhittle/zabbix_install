...
if (!(Test-Path $ChocoInstallPath)) {
  ...
  $chocoPkgFolder = get-childitem $resourcesPath -recurse -include 'chocolatey.*' | ?{$_.PSIsContainer} | sort-object -property Name -descending | select -First 1

  if ($chocoPkgFolder -ne $null) {
    # Run a completely offline install if everything is downloaded
    Write-Host "Installing chocolatey on this machine from `'$chocoPkgFolder`'"
